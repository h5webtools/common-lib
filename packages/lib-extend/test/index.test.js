...
describe('src/index.js', () => {
  it('should missing arguments', () => {
    extend(undefined, { a: 1 }).should.be.eql({ a: 1 }, 'missing first argument is second argument');
    extend({ a: 1 }).should.be.eql({ a: 1 }, 'missing second argument is first argument');
    extend(true, undefined, { a: 1 }).should.be.eql({ a: 1 }, 'deep: missing first argument is second argument');
    extend(true, { a: 1 }).should.be.eql({ a: 1 }, 'deep: missing second argument is first argument');
    extend().should.be.eql({}, 'no arguments is object');
  });
  ...
  it('should merge string with string', () => {
    ...
    target.should.be.eql(expectedTarget, 'string + string is merged object form of string');
  });

  it('should merge string with number', () => {
    const ori = 'what u gonna say';
    const target = extend(ori, 10);

    target.should.be.eql({}, 'string + number is empty object');
  });
  ...
  it('should merge string with array', () => {
    ...
    arr.should.be.eql([1, 'what', new Date(81, 8, 4)], 'array is unchanged');
    target.should.be.eql({
      0: 1,
      1: 'what',
      2: new Date(81, 8, 4)
  ...
  it('should merge string with date', () => {
    const ori = 'what u gonna say';
    const target = extend(ori, date);
    const testDate = new Date(81, 4, 13);
    
    date.should.be.eql(testDate, 'date is unchanged');
  });
});
...
//   t.deepEqual(
//     extend(true, target, source), [1, [2], { 3: true }],
